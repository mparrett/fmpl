...
**Phase:** ITER-0005a.2 DONE 2026-05-13 (STORY-0099 AC-5 writer-side sweep). All 4 currently-extant `save_to_fjall` call sites (`CompiledCode`, `ObjectDb` two-shape, `ParseState`, `grammar/stream_input.rs` spill+memo) route through `persistence::envelope::write`. New `(AC-5 ratchet)` at `tests/persistence_envelope_invariant.rs` (form #4 grep gate; principled rationale: `fjall::Keyspace::insert` is foreign-crate, can't be sealed at type level). EPIC-003 STORY-0099 status: 6/7 ACs done (1, 2, 3, 4, 5, 6); only AC-7 remains, scheduled in ITER-0005a.3.
**Iterations:** 16/17 done (ITER-0005a.2 closed; ITER-0005a.0 deferred). Pending: ITER-0004d.5 (blocked on io::read_dir), 0004e, 0004f, 0004g, 0005a.3, 0005b, 0005c, 0005d, 0005e, 0005f, 0006, 0007.
**Sentinel corpus (final, 2026-05-13 post-ITER-0005a.2):** under default features: ast_to_ir_parity 57/57 (2 ignored); scenario_0103 32/32 (1 ignored); tavern_demo 6/6; no_legacy_fmpl_syntax 16/16; diagnostics_fmpl_source_scan 32/32; canonical_pipeline_parity 8/8; opcode_rename_evidence 15/15; type_inference 13/13; scenario_runner 38/38; postlude_arm_contract 1/1; scenario_0099_envelope_loader 1/1; persistence_schema_anti_rot 2/2; persistence_envelope_invariant 3/3 (NEW). Plus the persistence-module's internal `#[cfg(test)]` tests: 33 (was 30 in 0005a.1; +3 `write` helper tests). **Grand total under default features: 1335 tests passing across 78 suites, 0 regressions** (+6 vs 0005a.1 baseline of 1329: +3 `write` helper + 3 AC-5 invariant gate). Under `--features fjall-persistence`: 1344 tests passing (the +9 are the gated `save_to_fjall`/`load_from_fjall` tests in `compiler.rs` + `object.rs` becoming live). Under `--features cross_compile`: 29 SCENARIO-0109 cases active. PARSER_EPOCH = 5. Canonical pipeline active.
**Last event:** 2026-05-13 — ITER-0005a.2 close. Writer-side sweep complete. T1-T5 implementation throughput averaged 211s/task (T1=122s T2=239s T3=79s T4=573s T5=43s) per `/tmp/iter-0005a.2-checkpoints/log.md`. AC-5 invariant gate proves all 4 sweep targets routed correctly. Pre-existing clippy warnings in `object_persistence.rs` (redundant closures + unused `id1`/`id2` vars) fixed inline — they were latent because clippy under `--features fjall-persistence --all-targets -- -D warnings` wasn't a gate before. All 11 PAR-revision findings (2 Critical + 9 Serious) closed. ITER-0005a.3 (load-side rewire + AC-7 LoaderStats public API) is next pending — 4 transitional `// TODO(ITER-0005a.3)` markers in `compiler.rs`, `object.rs`, `grammar/incremental.rs`, `grammar/stream_input.rs` mark its work surface.
