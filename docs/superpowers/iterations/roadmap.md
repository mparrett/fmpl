...
**Build-order dependency (binding):** Scope items execute in this order: **1 → 2 → 7 → 3 → 4 → 5 → 6 → 8**. **Item 7 (cleanup deletions) BEFORE item 3 (migration)** so the migration set is reduced before transformer invocation — avoids wasted migration on files about to be deleted (`ast_to_ir_indexed.fmpl`, `ir_to_execution_tape_indexed.fmpl`, `pipeline_demo.fmpl`). **Item 3 SHOULD precede item 4 (lib.rs wiring)** — note (round-3 PAR correction): functional correctness does NOT strictly require this order. `Pattern::TagMatch` at `fmpl-core/src/grammar/runtime.rs:794` matches list_node shape and `Instruction::MakeTagged` at `fmpl-core/src/vm.rs:877` constructs list_node post-ITER-0004b, so legacy `:Tag(args)` patterns in the un-migrated optimizer would still fold against `Value::list_node` AST input. The migration is needed for AC-13 (greppable invariant) and to keep `ast_optimizer.fmpl` consistent with the canonical syntax. Item 3 before item 4 is preferred for clean bisect (one concern per commit) and AC-13 stability during the wiring window, not for optimizer correctness.

**Scope:**

1. **Build the FMPL transformer** (`tools/list-transform/list_transform.fmpl` + driver). Was Phase A item 3 in ITER-0004b's plan. Tree-grammar rules + special-case rules — explicitly: (a) trailing comma for single-element list patterns, (b) pair sentinel wrap, (c) list-pattern binding repair, (d) **guard preservation** — `&{...}` guard syntax follows the LHS pattern in grammar rules (e.g. `ast_optimizer.fmpl:5` has `:Binary(:/, :Int(a), :Int(b)) &{ b != 0 } => :Int(a / b)`); the transformer must preserve guards lexically as it rewrites the LHS pattern, (e) **string-literal context exclusion** (do not rewrite inside `"..."`), (f) **comment context** — for the FMPL transformer, treat `--`-prefixed comments as opaque text and do NOT rewrite them; comment hand-edits are handled in scope item 3. Driver in Rust (~50 lines) walks the targets in scope item 3.
...
3. **Apply the FMPL transformer** to the 5 in-scope migration files: `ast_optimizer.fmpl`, `fmpl_parser.fmpl`, `ir_to_rust.fmpl`, `prelude.fmpl`, `ir_to_execution_tape.fmpl`. Transformer skips comments and string literals; see item 1.f. The other previously-listed files (`ast_to_ir_indexed.fmpl`, `ir_to_execution_tape_indexed.fmpl`, `pipeline_demo.fmpl`) are deleted in scope item 7 and are NOT migrated. **Order:** scope item 7's deletions execute BEFORE scope item 3 to avoid wasted migration on files about to be deleted (revised build order: 1 → 2 → 7 → 3 → 4 → 5 → 6 → 8). Re-run the transformer until idempotent. Hand-edit transformer-flagged exceptions, AND ALSO hand-edit comment lines and string-literal lines that contain legacy `:Tag(args)` syntax (the transformer skips these contexts).
